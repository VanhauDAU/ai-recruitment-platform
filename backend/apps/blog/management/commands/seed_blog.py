...
SETTINGS = [
    ('blog_page_title', 'Tiêu đề trang Cẩm nang', T.TEXT, 'Cẩm nang nghề nghiệp', True,
     'Tiêu đề hiển thị ở đầu trang /blog và title tag.'),
    ('blog_meta_description', 'Mô tả SEO trang Cẩm nang', T.TEXTAREA,
     'Cẩm nang nghề nghiệp: định hướng, bí kíp tìm việc, lương thưởng và kiến thức chuyên ngành.', True,
     'Meta description mặc định cho trang danh sách blog.'),
    ('blog_support_docs_title', 'Tiêu đề khối Tài liệu hỗ trợ', T.TEXT, 'Tài liệu hỗ trợ tìm việc', True,
     'Tiêu đề khối bài viết ghim ở sidebar trang chi tiết.'),
    ('blog_benefits', 'Khối lợi ích (trang chi tiết bài viết)', T.JSON, {
        'items': [
        ...
        'cta_url': '/',
    }, True, 'Khối "Lợi ích khi sử dụng <tên site>" đầu trang chi tiết bài viết: items [{title, description}] + nút CTA.'),
]

# Quyền theo nhóm nhân viên.
EDITOR_CODENAMES = ['add_post', 'change_post', 'view_post', 'view_postcategory', 'view_tag']
MANAGER_CODENAMES = [
    'add_post', 'change_post', 'delete_post', 'view_post', 'can_publish_post',
    'add_postcategory', 'change_postcategory', 'delete_postcategory', 'view_postcategory',
    'add_tag', 'change_tag', 'delete_tag', 'view_tag',
    'add_pinnedpost', 'change_pinnedpost', 'delete_pinnedpost', 'view_pinnedpost',
]
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for key, label, value_type, default, is_public, description in SETTINGS:
            metadata = {'label': label, 'group': G.BLOG, 'value_type': value_type,
                        'is_public': is_public, 'description': description}
            setting, created = SiteSetting.objects.get_or_create(key=key, defaults={**metadata, 'value': default})
            if not created:
    ...
    def _sync_group(self, group_name, codenames):
        content_types = ContentType.objects.get_for_models(Post, PostCategory, Tag, PinnedPost).values()
        ...
        self.stdout.write(f'{"+ " if created else "= "}group {group_name} ({len(found)} quyền){note}')
