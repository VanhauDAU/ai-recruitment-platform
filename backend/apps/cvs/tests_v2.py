...
class CvV2ApiTests(APITestCase):
    ...
    def setUp(self):
        self.candidate = get_user_model().objects.create_user(
            email='v2-candidate@example.com',
            password='password',
            role='candidate',
            email_verified=True,
        )
        self.other_candidate = get_user_model().objects.create_user(
            email='v2-other@example.com',
            password='password',
            role='candidate',
            email_verified=True,
        )
    ...
    def export_url(self, cv, export, action=None):
        ...
        return reverse(
            name, kwargs={'public_id': cv.public_id, 'export_public_id': export.public_id}
        )

    def create_alternate_template(self):
        template = CvTemplate.objects.create(
            name='Two column V2 Template',
            lifecycle_status=CvTemplate.LifecycleStatus.PUBLISHED,
        )
        layout = empty_layout()
        layout['regions'] = [
    ...
    def test_latest_recoverable_draft_uses_hash_and_clears_after_manual_save(self):
        ...
        self.assertEqual(
            recoverable.data['draft']['content_json']['personal_info']['full_name'], 'Recover me'
        )
    ...
    def test_create_can_copy_owned_draft_into_a_new_template(self):
        ...
        self.assertEqual(
            copied.draft.content_json['personal_info']['full_name'], 'Preserved source identity'
        )
    ...
    def test_import_hard_delete_and_metadata_use_v2_without_legacy_storage_urls(self):
        uploaded = SimpleUploadedFile(
            'source.pdf', b'%PDF-1.4 test', content_type='application/pdf'
        )
    ...
    def test_template_import_is_idempotent_and_materializes_editable_canonical_draft(
        self, structure_cv_text
    ):
        payload = {
        ...
        with patch('apps.cvs.tasks.process_cv_import_job.delay'):
            ...
            second = self.client.post(
                reverse('cv-v2-import'),
                {**payload, 'file': self._docx_upload()},
                format='multipart',
                HTTP_IDEMPOTENCY_KEY='same-import',
            )
        ...
        self.assertEqual(cv.latest_version.version_kind, CvVersion.VersionKind.IMPORTED)
        self.assertEqual(
            cv.draft.content_json['personal_info']['full_name'], 'Nguyen Van Candidate'
        )
        self.assertEqual(cv.draft.style_json['theme_color'], '#2255AA')
    ...
    def test_import_rejects_spoofed_file_signature_before_persisting(self):
        upload = SimpleUploadedFile(
            'spoofed.pdf', b'not a real pdf', content_type='application/pdf'
        )
        response = self.client.post(
            reverse('cv-v2-import'),
    ...
    def test_failed_import_exposes_safe_code_and_can_retry(self):
        ...
        with patch(
            'apps.cvs.tasks._extract_import_text',
            side_effect=ImportProcessingError('scanned_pdf_ocr_unavailable'),
        ):
            process_cv_import_job(cv.import_job.pk)
    ...
    def test_duplicate_creates_an_independent_builder_draft_from_the_latest_version(self):
        ...
        self.assertEqual(
            duplicate.current_template_version_id, source_cv.current_template_version_id
        )
        self.assertEqual(
            duplicate.draft.content_json['personal_info']['full_name'], 'Original candidate'
        )
        ...
        source_cv.refresh_from_db()
        self.assertEqual(
            source_cv.draft.content_json['personal_info']['full_name'], 'Original candidate'
        )
    ...
    def test_template_switch_preserves_content_and_changes_only_mutable_presentation(self):
        ...
        self.assertEqual(
            response.data['cv']['template_capabilities']['layout']['column_resize']['min_percent'],
            25,
        )
    ...
    def test_layout_item_order_is_validated_and_autosave_keeps_canonical_item_data(self):
        ...
        self.assertEqual(
            autosave.data['layout_json']['item_orders']['experience_1'],
            ['experience_item_2', 'experience_item_1'],
        )
    ...
    def test_owner_view_and_shared_link_are_pinned_to_an_immutable_version_and_audited(self):
        ...
        self.client.put(
            self.draft_url(cv), save_payload, format='json', HTTP_IF_MATCH='"lock-version-0"'
        )
        saved = self.client.post(
            reverse('cv-v2-save-version', kwargs={'public_id': cv.public_id}),
            format='json',
        ...
        self.client.put(
            self.draft_url(cv), mutable_payload, format='json', HTTP_IF_MATCH='"lock-version-1"'
        )

        owner_view = self.client.get(
            self.owner_view_url(cv),
            REMOTE_ADDR='203.0.113.9',
            HTTP_USER_AGENT='owner-browser',
        )
        ...
        self.assertEqual(
            owner_view.data['version']['content_json']['personal_info']['full_name'],
            'Immutable Candidate',
        )
        ...
        self.assertGreaterEqual(
            len(raw_token), 43
        )  # token_urlsafe(32): at least 256 bits of entropy
    ...
    def test_shared_link_expiry_revoke_and_invalid_tokens_return_404(self):
        ...
        self.assertEqual(
            self.client.get(
                reverse('cv-v2-shared-link-public', kwargs={'token': 'not-a-real-token'})
            ).status_code,
            404,
        )
        ...
        CvSharedLink.objects.filter(pk=link.pk).update(
            expires_at=timezone.now() - timedelta(seconds=1)
        )
        self.assertEqual(
            self.client.get(
                reverse('cv-v2-shared-link-public', kwargs={'token': expiring.data['token']})
            ).status_code,
            404,
        )
    ...
    def test_owner_view_and_shared_link_management_block_idor(self):
        ...
        self.assertEqual(
            self.client.post(self.shared_links_url(cv), format='json').status_code, 404
        )
        self.assertEqual(
            self.client.delete(
                reverse(
    ...
    def test_candidate_ownership_and_employer_access_are_enforced(self):
        ...
        self.assertEqual(
            self.client.get(
                reverse('cv-v2-version-list', kwargs={'public_id': cv.public_id})
            ).status_code,
            404,
        )

        employer = get_user_model().objects.create_user(
            email='v2-employer@example.com',
            password='password',
            role='employer',
        )
    ...
    def test_saved_versions_are_read_only_and_save_rolls_back_if_version_insert_fails(self):
        ...
        response = self.client.patch(
            reverse(
                'cv-v2-version-detail',
                kwargs={
                    'public_id': cv.public_id,
                    'version_public_id': cv.latest_version.public_id,
                },
            ),
            {'plain_text': 'attempted mutation'},
            format='json',
        )
        self.assertEqual(response.status_code, 405)

        with patch(
            'apps.cvs.services.versions.CvVersion.save',
            ...
            with self.assertRaisesRegex(RuntimeError, 'database failure'):
                self.client.post(
                    reverse('cv-v2-save-version', kwargs={'public_id': cv.public_id}),
                    format='json',
                    HTTP_IF_MATCH=f'"lock-version-{draft["lock_version"]}"',
                )
        cv.refresh_from_db()
        self.assertEqual(CvVersion.objects.filter(cv=cv).count(), 1)
    ...
    def test_creation_requires_verified_email_and_a_published_template_version(self):
        unverified = get_user_model().objects.create_user(
            email='v2-unverified@example.com',
            password='password',
            role='candidate',
        ...
        response = self.client.post(
            reverse('cv-v2-list-create'),
            {'title': 'Blocked', 'template_public_id': self.template.public_id},
            format='json',
        )
        ...
        response = self.client.post(
            reverse('cv-v2-list-create'),
            {'title': 'No release', 'template_public_id': self.template.public_id},
            format='json',
        )
        self.assertEqual(response.status_code, 403)
    ...
    def test_create_from_sample_builds_usercv_initial_version_and_draft(self):
        ...
        self.assertEqual(
            cv.latest_version.content_json['personal_info']['full_name'], 'Sample Person'
        )
    ...
    def test_apply_sample_uses_cas_and_preserves_personal_info_and_style(self):
        ...
        self.assertEqual(
            applied.data['content_json']['personal_info']['full_name'], 'Tên ứng viên thật'
        )
        self.assertEqual(
            applied.data['content_json']['sections'][0]['items'][0]['value'], 'Nội dung mẫu'
        )
    ...
    def test_avatar_upload_reencodes_image_and_enforces_owner_access(self):
        ...
        created = self.client.post(
            reverse('cv-v2-asset-upload'), {'file': upload}, format='multipart'
        )
        ...
        rejected = self.client.post(
            reverse('cv-v2-asset-upload'), {'file': invalid}, format='multipart'
        )
    ...
    def test_create_from_position_resolves_blueprint_and_persists_taxonomy_identity(self):
        ...
        JobCategoryLocalization.objects.bulk_create(
            [
                JobCategoryLocalization(
                    category=position, locale='vi-VN', display_name='Nhân viên CSKH'
                ),
                JobCategoryLocalization(
                    category=position,
                    locale='en-US',
        ...
        self.assertEqual(
            cv.draft.content_json['personal_info']['headline'], 'Customer Service Representative'
        )
    ...
    def test_pdf_export_uses_a_selected_immutable_version_not_the_mutable_draft(self):
        ...
        self.client.put(
            self.draft_url(cv), saved_payload, format='json', HTTP_IF_MATCH='"lock-version-0"'
        )
        saved = self.client.post(
            reverse('cv-v2-save-version', kwargs={'public_id': cv.public_id}),
            format='json',
        ...
        self.assertEqual(
            export.version.content_json['personal_info']['full_name'], 'Frozen Export Candidate'
        )
        self.assertEqual(
            cv.draft.content_json['personal_info']['full_name'], 'Draft Must Never Export'
        )
    ...
    def test_pdf_export_reuses_valid_artifact_and_download_url_is_owner_controlled(self):
        ...
        self.assertTrue(
            reused.data['download_url'].endswith(self.export_url(cv, export, 'download'))
        )
    ...
    def test_pdf_export_worker_marks_failure_and_retry_keeps_the_same_version(self):
        ...
        with patch(
            'apps.cvs.tasks.render_cv_version_pdf', side_effect=RuntimeError('renderer unavailable')
        ):
    ...
    def test_pdf_export_worker_stores_a_private_pdf_artifact_from_version_only(self):
        ...
        export = CvExport.objects.get(public_id=created.data['public_id'])
        with patch(
            'apps.cvs.tasks.render_cv_version_pdf', return_value=b'%PDF-1.4 rendered version'
        ) as renderer:
            render_cv_export_job(export.pk)
        export.refresh_from_db()
        self.assertEqual(export.status, CvExport.Status.COMPLETED)
        self.assertTrue(
            export.storage_key.startswith(f'cvs/exports/{cv.public_id}/{export.version.public_id}/')
        )
    ...
    def test_pdf_export_blocks_idor_and_foreign_version_selection(self):
        ...
        response = self.client.post(
            self.exports_url(cv),
            {'version_public_id': other_version.public_id},
            format='json',
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(CvExport.objects.filter(cv=cv).count(), 1)
        ...
        employer = get_user_model().objects.create_user(
            email='export-employer@example.com',
            password='password',
            role='employer',
        )
        self.client.force_authenticate(employer)
    ...
    def test_private_thumbnail_is_generated_from_latest_version_and_owner_scoped(
        self, _render, _raster
    ):
        cv = self.create_cv()
        cv.refresh_from_db()
        ...
        self.assertEqual(
            self.client.get(
                reverse('cv-v2-thumbnail', kwargs={'public_id': cv.public_id})
            ).status_code,
            404,
        )
