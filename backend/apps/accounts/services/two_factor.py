...
def verify_code(user, purpose, code):
    ...
    if not expected or not code or not secrets.compare_digest(str(expected), str(code)):
        return False
    cache.delete(_code_key(user.pk, purpose))
    cache.delete(_expiry_key(user.pk, purpose))
    return True
