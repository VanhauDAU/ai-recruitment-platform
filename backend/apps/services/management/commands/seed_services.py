...
CATEGORIES = [
    ('featured-jobs', 'Tin tuyển dụng nổi bật', 'Featured job postings',
     ...
     'ThunderboltOutlined'),
    ('combo', 'Combo tiết kiệm', 'Saver combos',
    ...
    ('ai-credits', 'AI & Credits', 'AI & Credits',
     ...
     'RobotOutlined'),
    ('branding', 'Employer Branding', 'Employer Branding',
     ...
     'StarOutlined'),
    ('addons', 'Dịch vụ bổ trợ', 'Add-on services',
...
PACKAGES = [
    ('featured-jobs', 'top-max', 'TOP MAX', 'TOP MAX',
     'Đăng tin hiệu suất cao, phủ sóng tối đa', 'High-performance posting with maximum reach',
     7500000, '/ tin 30 ngày', '/ posting, 30 days',
     ...
     'Hiệu quả nhất', 'Best value', True, ServicePackage.CtaType.CONTACT),
    ('featured-jobs', 'top-max-plus', 'TOP MAX PLUS', 'TOP MAX PLUS',
     'Phiên bản mở rộng với số lần đẩy TOP gấp đôi', 'Extended version with double top boosts',
     9650000, '/ tin 30 ngày', '/ posting, 30 days',
     ...
     '', '', False, ServicePackage.CtaType.CONTACT),
    ('featured-jobs', 'top-eco-plus', 'TOP ECO PLUS', 'TOP ECO PLUS',
     'Lựa chọn tiết kiệm cho tin cần độ phủ vừa phải', 'Economical choice for moderate reach',
     4400000, '/ tin 30 ngày', '/ posting, 30 days',
    ...
    ('combo', 'combo-starter', 'Combo Starter', 'Starter Combo',
     'Gói 3 tin tiêu chuẩn cho doanh nghiệp mới bắt đầu', 'Three standard postings for new businesses',
     2550000, '/ 3 tin', '/ 3 postings',
     ...
     '', '', False, ServicePackage.CtaType.CONTACT),
    ('combo', 'combo-growth', 'Combo Growth', 'Growth Combo',
     'Giải pháp trọn gói cho nhu cầu tuyển dụng liên tục', 'All-in bundle for continuous hiring',
     None, '', '',
    ...
    ('ai-credits', 'ai-cv-screening', 'AI sàng lọc CV', 'AI CV Screening',
     'Tự động chấm điểm và xếp hạng hồ sơ ứng tuyển', 'Automatically score and rank applications',
     3000000, '/ tháng', '/ month',
     ...
     'AI', 'AI', True, ServicePackage.CtaType.CONTACT),
    ('ai-credits', 'ai-matching-credits', 'Credit gợi ý ứng viên', 'Candidate Matching Credits',
     'Chủ động tiếp cận ứng viên phù hợp từ kho hồ sơ', 'Proactively reach matching candidates',
     5000000, '/ 100 credits', '/ 100 credits',
    ...
    ('branding', 'branding-banner', 'Banner trang chủ', 'Homepage Banner',
     'Banner thương hiệu trên trang chủ và trang tìm việc', 'Brand banner on homepage and job search',
     8000000, '/ 4 tuần', '/ 4 weeks',
     ...
     '', '', False, ServicePackage.CtaType.CONTACT),
    ('branding', 'branding-top-employer', 'Trang tuyển dụng thương hiệu', 'Branded Career Page',
     'Chuyên trang tuyển dụng riêng với giao diện khác biệt', 'Dedicated career page with custom look',
     None, '', '',
    ...
    ('addons', 'job-refresh', 'Làm mới tin đăng', 'Job Refresh',
     'Đẩy tin lên đầu danh sách như tin mới đăng', 'Push your posting back to the top',
     500000, '/ lần', '/ refresh',
     ['Tin hiển thị như mới đăng',
      'Tăng lượt xem trong 24h đầu',
      'Kích hoạt ngay khi cần'],
     ['Posting appears as newly published',
      'More views within the first 24h',
      'Activate instantly whenever needed'],
     '', '', False, ServicePackage.CtaType.CONTACT),
    ('addons', 'urgent-tag', 'Nhãn TUYỂN GẤP', 'URGENT Label',
     'Gắn nhãn nổi bật vào tiêu đề tin tuyển dụng', 'Add an eye-catching label to your posting title',
     1000000, '/ tin', '/ posting',
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        categories = {}
        for order, (key, name_vi, name_en, desc_vi, desc_en, icon) in enumerate(CATEGORIES, start=1):
            category, created = ServiceCategory.objects.get_or_create(
                key=key,
                defaults={
                    'name_vi': name_vi, 'name_en': name_en,
                    'description_vi': desc_vi, 'description_en': desc_en,
        ...
        for order, (category_key, slug, name_vi, name_en, tagline_vi, tagline_en, price,
                    unit_vi, unit_en, benefits_vi, benefits_en, badge_vi, badge_en,
                    is_highlight, cta_type) in enumerate(PACKAGES, start=1):
            _, created = ServicePackage.objects.get_or_create(
                slug=slug,
                defaults={
                    'category': categories[category_key],
                    'name_vi': name_vi, 'name_en': name_en,
                    'tagline_vi': tagline_vi, 'tagline_en': tagline_en,
                    'price': price, 'unit_vi': unit_vi, 'unit_en': unit_en,
                    'benefits_vi': benefits_vi, 'benefits_en': benefits_en,
                    'badge_vi': badge_vi, 'badge_en': badge_en,
                    'is_highlight': is_highlight, 'cta_type': cta_type,
                    'order': order,
                },
            )
