...
class UploadDeploymentContractTests(SimpleTestCase):
    def test_every_routed_queue_is_consumed_by_the_compose_worker(self):
        repository_root = Path(__file__).resolve().parents[4]
        compose_path = repository_root / 'docker-compose.yml'
        if not compose_path.exists():
            self.skipTest('Repository root is not mounted in this backend runtime.')
        compose = compose_path.read_text(encoding='utf-8')
        worker_queue_match = re.search(r'celery -A config worker[^\n]* -Q ([^\s]+)', compose)

        self.assertIsNotNone(worker_queue_match)
        consumed_queues = set(worker_queue_match.group(1).split(','))
        routed_queues = {
            route['queue']
        ...
        self.assertTrue(
            ...
            f'Celery worker does not consume routed queues: {routed_queues - consumed_queues}',
        )
