...
function DraggableSectionChip({ sectionKey, locale, onAdd }) {
  const draggable = useDraggable({ id: `new:${sectionKey}` })
  return <button ref={draggable.setNodeRef} {...draggable.attributes} {...draggable.listeners} type="button" onClick={() => onAdd(sectionKey)} className={`flex min-h-16 cursor-grab items-center justify-between gap-1 rounded-lg border border-dashed border-emerald-300 bg-emerald-50 p-2 text-left text-sm font-semibold text-emerald-800 transition hover:border-emerald-500 hover:bg-emerald-100 ${draggable.isDragging ? 'opacity-40' : ''}`}>
    <span>+ {getSectionDisplayName(sectionKey, locale)}</span>
    <HolderOutlined className="shrink-0 text-emerald-400" aria-hidden="true" />
  </button>
}
...
export default function AddSectionsPanel({ locale, availableKeys, sections, onAdd, onAddCustom, onLocate }) {
  ...
  return <div className="space-y-6">
    <section>
      ...
      <p className="mb-3 text-xs text-slate-500">Bấm để chèn mục sau phần đang chọn, hoặc kéo thả thẳng vào vị trí mong muốn trên CV.</p>
      <div className="grid grid-cols-2 gap-2">{availableKeys.filter((key) => key !== 'custom').map((key) => <DraggableSectionChip key={key} sectionKey={key} locale={locale} onAdd={onAdd} />)}</div>
