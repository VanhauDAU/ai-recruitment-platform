...
export default function BuilderTopBar({ editor, onPreview, onSave }) {
  return <header aria-label="Thanh hành động CV" className="z-40 flex h-[52px] shrink-0 items-center justify-between gap-3 border-b border-slate-200 bg-white px-3 md:px-5">
    ...
    <div className="flex shrink-0 items-center gap-1.5">
      <span className="hidden md:block"><EditorSaveState compact phase={editor.phase} savedAt={editor.savedAt} /></span>
      ...
      <Button aria-label="Lưu CV" type="primary" shape="round" icon={<SaveOutlined />} loading={editor.phase === 'saving'} disabled={editor.phase === 'conflict'} onClick={onSave}>Lưu CV</Button>
      <Dropdown menu={{ items: [
        { key: 'export', label: <Link to={`/cvs/${editor.cv.public_id}/view`}>Xuất PDF</Link>, icon: <DownloadOutlined /> },
      ] }}><Button type="text" aria-label="Thao tác khác" icon={<MoreOutlined />} /></Dropdown>
    </div>
  </header>
}
