    ...
    <section aria-labelledby="layout-resize-title" className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 space-y-3">
        {regions.slice(0, -1).map((region, index) => {
          ...
          return <label key={region.id} className="block"><span className="mb-1 flex justify-between text-sm font-medium text-slate-700"><span>{getRegionLabel(region.id)} / {getRegionLabel(adjacent.id)}</span><span>{region.width_percent}% / {adjacent.width_percent}%</span></span><input aria-label={`Tỷ lệ ${region.id}`} type="range" min={min} max={max} step="1" value={region.width_percent} onChange={(event) => onResize(region.id, Number(event.target.value))} className="w-full accent-[var(--brand-primary)]" /></label>
        })}
      </div>
    </section>
  )
