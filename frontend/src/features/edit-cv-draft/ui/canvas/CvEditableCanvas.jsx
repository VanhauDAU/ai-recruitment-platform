...
import { CvDocumentPreview, getCvUiText, getOrderedItems, getRendererContract, getSectionDefinition, uploadCvAsset } from '@/entities/cv'
...
function SortableItem({ item, section, locale, onChange, onRemove, onMove, registerPendingEdit }) {
  ...
  return <div ref={sortable.setNodeRef} data-cv-item-id={`${section.instance_id}:${item.item_id}`} style={{ transform: CSS.Transform.toString(sortable.transform), transition: sortable.transition }} className={`group/item relative mb-3 rounded-md p-1 hover:bg-emerald-50 ${sortable.isDragging ? 'opacity-40' : ''}`}>
    ...
    {fields.filter((field) => field in item).map((field) => <div key={field} className={titleFields.has(field) ? 'font-bold text-slate-800' : 'text-slate-600'}><InlineText value={item[field] || ''} placeholder={getCvUiText(field, locale)} ariaLabel={`${field} ${item.item_id}`} onCommit={(value) => onChange({ [field]: value })} registerPendingEdit={registerPendingEdit} /></div>)}
...
function SortableSection({ section, document, personal, assets, pageNumber, onRename, onItemChange, onAddItem, onRemoveItem, onMoveItem, onMoveSection, onRemoveSection, registerPendingEdit, onPersonalChange }) {
  ...
  if (section.section_key === 'nameplate') return <section ref={sortable.setNodeRef} style={style} className="group/section relative mb-5">{handle}<InlineText value={personal.full_name || ''} placeholder={getCvUiText('full_name', locale)} ariaLabel="Họ và tên inline" className="text-2xl font-extrabold" onCommit={(full_name) => onPersonalChange({ full_name })} registerPendingEdit={registerPendingEdit} /><div><InlineText value={personal.headline || ''} placeholder={getCvUiText('headline', locale)} ariaLabel="Chức danh inline" className="font-semibold text-[var(--cv-theme)]" onCommit={(headline) => onPersonalChange({ headline })} registerPendingEdit={registerPendingEdit} /></div></section>
  if (section.section_key === 'contact') return <section ref={sortable.setNodeRef} style={style} className="group/section relative mb-5 text-slate-600">{handle}<InlineText value={personal.email || ''} placeholder={getCvUiText('email', locale)} ariaLabel="Email inline" onCommit={(email) => onPersonalChange({ email })} registerPendingEdit={registerPendingEdit} /> · <InlineText value={personal.phone || ''} placeholder={getCvUiText('phone', locale)} ariaLabel="Số điện thoại inline" onCommit={(phone) => onPersonalChange({ phone })} registerPendingEdit={registerPendingEdit} /> · <InlineText value={personal.address || ''} placeholder={getCvUiText('address', locale)} ariaLabel="Địa chỉ inline" onCommit={(address) => onPersonalChange({ address })} registerPendingEdit={registerPendingEdit} /></section>
  if (section.section_key === 'avatar') {
    const avatar = assets?.[personal.avatar_asset_id]
    return <section ref={sortable.setNodeRef} style={style} className="group/section relative mb-5">{handle}<Upload showUploadList={false} accept="image/jpeg,image/png,image/webp" beforeUpload={async (file) => { try { const uploaded = await uploadCvAsset(file); onPersonalChange({ avatar_asset_id: uploaded.public_id }, uploaded); message.success('Đã cập nhật ảnh đại diện') } catch { message.error('Không thể tải ảnh đại diện') } return false }}><button type="button" className="rounded-full focus:ring-2 focus:ring-emerald-400">{avatar?.url ? <img src={avatar.url} alt={getCvUiText('upload_avatar', locale)} className="h-24 w-24 rounded-full object-cover" /> : <span className="flex h-24 w-24 items-center justify-center rounded-full bg-slate-200 text-xs">{getCvUiText('upload_avatar', locale)}</span>}</button></Upload></section>
  }
  ...
  return <section id={pageNumber === 1 ? `cv-section-${section.instance_id}` : undefined} ref={sortable.setNodeRef} style={style} className={`group/section relative mb-5 rounded p-1 hover:bg-emerald-50/40 ${sortable.isDragging ? 'opacity-40' : ''}`}>
    {handle}
    <div className="mb-2 flex items-center gap-2 border-b pb-1 text-sm font-bold uppercase tracking-wide text-[var(--cv-theme)]"><InlineText value={section.title || definition?.displayName || ''} placeholder={getCvUiText('section_title', locale)} ariaLabel={`Tiêu đề ${section.instance_id}`} onCommit={onRename} registerPendingEdit={registerPendingEdit} /><div className="ml-auto flex gap-1 rounded bg-white/90 lg:pointer-events-none lg:opacity-0 lg:group-hover/section:pointer-events-auto lg:group-hover/section:opacity-100 lg:group-focus-within/section:pointer-events-auto lg:group-focus-within/section:opacity-100"><button type="button" aria-label={`Đưa section ${section.instance_id} lên`} onClick={() => onMoveSection(-1)}><ArrowUpOutlined /></button><button type="button" aria-label={`Đưa section ${section.instance_id} xuống`} onClick={() => onMoveSection(1)}><ArrowDownOutlined /></button>{definition?.deletable !== false && <button type="button" aria-label={`Xóa section ${section.instance_id}`} onClick={onRemoveSection} className="text-rose-500"><DeleteOutlined /></button>}</div></div>
    <SortableContext items={items.map((item) => `item:${section.instance_id}:${item.item_id}`)} strategy={verticalListSortingStrategy}>{items.map((item) => <SortableItem key={item.item_id} item={item} section={section} locale={locale} onChange={(patch) => onItemChange(item.item_id, patch)} onRemove={() => onRemoveItem(item.item_id)} onMove={(direction) => onMoveItem(item.item_id, direction)} registerPendingEdit={registerPendingEdit} />)}</SortableContext>
    {definition?.requiresItems && <Button size="small" type="dashed" icon={<PlusOutlined />} onClick={onAddItem}>{getCvUiText('add_item', locale)}</Button>}
  </section>
}
...
export default function CvEditableCanvas({ editor, zoom, onMoveSection, onMoveItem, onMoveItemDirection, onMoveSectionDirection, onRenameSection, onItemChange, onAddItem, onRemoveItem, onRemoveSection, onPersonalChange }) {
  ...
  const header = () => contract.key !== 'header_two_column_v1' ? <header className="mb-6 border-l-4 pl-4" style={{ borderColor: editor.document.style_json.theme_color }}><InlineText value={personal.full_name || ''} placeholder={getCvUiText('full_name', locale)} ariaLabel="Họ và tên inline" className="text-2xl font-extrabold" onCommit={(full_name) => onPersonalChange({ full_name })} registerPendingEdit={editor.registerPendingEdit} /><div><InlineText value={personal.headline || ''} placeholder={getCvUiText('headline', locale)} ariaLabel="Chức danh inline" className="font-semibold text-[var(--cv-theme)]" onCommit={(headline) => onPersonalChange({ headline })} registerPendingEdit={editor.registerPendingEdit} /></div><div className="text-slate-600"><InlineText value={personal.email || ''} placeholder={getCvUiText('email', locale)} ariaLabel="Email inline" onCommit={(email) => onPersonalChange({ email })} registerPendingEdit={editor.registerPendingEdit} /> · <InlineText value={personal.phone || ''} placeholder={getCvUiText('phone', locale)} ariaLabel="Số điện thoại inline" onCommit={(phone) => onPersonalChange({ phone })} registerPendingEdit={editor.registerPendingEdit} /></div></header> : null
