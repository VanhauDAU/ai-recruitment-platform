...
export default function TemplateSwitcher({ currentTemplatePublicId, locale, disabled, onSwitch, currentSections }) {
  ...
  const confirmTemplateSwitch = async () => {
    if (!confirmation) return
    setLoading(true)
    try {
      const result = await onSwitch(confirmation.template.public_id)
  ...
  return (
    <section aria-labelledby="template-switch-title" className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <ConfirmActionModal open={Boolean(confirmation)} title="Đổi mẫu CV" confirmText="Đổi mẫu" cancelText="Quay lại" confirmLoading={loading} onCancel={() => setConfirmation(null)} onConfirm={confirmTemplateSwitch}><p>Bạn có chắc muốn đổi sang <strong>{confirmation?.template.display_name || 'mẫu CV này'}</strong> không?</p><p className="mt-1">Nội dung CV được giữ nguyên; bố cục cuối cùng sẽ được cập nhật theo mẫu mới.</p>{confirmation?.hidden.length > 0 ? <div className="mx-auto mt-3 max-w-sm text-left"><p className="font-semibold text-amber-700">Các mục sau được bảo toàn nhưng tạm ẩn:</p><ul className="mt-1 list-disc pl-5">{confirmation.hidden.map((section) => <li key={section.instance_id}>{section.title}</li>)}</ul></div> : <p className="mt-3 text-emerald-700">Mẫu mới hỗ trợ tất cả mục đang dùng.</p>}</ConfirmActionModal>
    </section>
  )
}
