...
function BackupCodesModal({ codes, email, brandName, onClose }) {
  const copyCodes = async () => {
    await navigator.clipboard?.writeText(codes.join('\n'))
    message.success('Đã sao chép mã dự phòng.')
  }
  const downloadCodes = () => {
    const content = [`Backup Codes – ${brandName}`, '', 'Giữ các mã này ở nơi an toàn.', 'Mỗi mã chỉ sử dụng được một lần.', '', ...codes.map((code) => `- ${code}`), '', 'Nếu bạn làm mất các mã này, vui lòng tạo mã mới trong phần Cài đặt.'].join('\n')
    const url = URL.createObjectURL(new Blob([content], { type: 'text/plain;charset=utf-8' }))
    const link = document.createElement('a')
    link.href = url
    const slug = brandName.normalize('NFD').replace(/[^a-z0-9]+/gi, '-').toUpperCase()
    link.download = `${slug}-BACKUP-CODES-${String(email || 'account').replace(/[^a-z0-9@._-]/gi, '_')}.txt`
    link.click()
...
export default function TwoFactorMethodsPanel({
  ...
  async function confirmEmailAction(code) {
    const response = verification.action === 'method-disable'
      ? await disableMfaMethod(verification.target, verification.method, code)
      : verification.action === 'email-disable'
      ? await confirmTwoFactorDisable(code)
      : verification.action === 'backup'
        ? await generateBackupCodes(code, verification.method || 'email')
  ...
  async function resendEmailAction() {
    ...
    const response = action === 'method-disable'
      ? await sendMfaMethodDisableCode(verification.target)
  ...
  async function startMethodDisable(target) {
    ...
    setSending(true)
    try {
      const response = await sendMfaMethodDisableCode(target)
  ...
  async function startTotp() {
    setSending(true)
    try {
      setTotpSetup(await startTotpSetup())
      setTotpCode('')
      setTotpError('')
    } catch (error) {
  ...
  async function confirmTotp() {
    ...
    setTotpError('')
    try {
      const response = await confirmTotpSetup(totpCode)
      applyUserResponse(response)
      setTotpSetup(null)
