...
export default function JobList() {
  ...
  return (
    <section className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-2xl font-extrabold text-slate-900">Tin tuyển dụng</h1><p className="mt-1 text-sm text-slate-500">Chỉ bạn có thể xem và quản lý các tin mình đã tạo.</p></div><Button type="primary" icon={<PlusOutlined />}><Link to={employerAppPath('/jobs/new')}>Đăng tin</Link></Button></div>
      ...
      <Table rowKey="public_id" loading={jobsQuery.isLoading} dataSource={jobsQuery.data || []} pagination={false} scroll={{ x: 760 }} locale={{ emptyText: <Empty description="Chưa có tin tuyển dụng" /> }} columns={[
        { title: 'Tin tuyển dụng', dataIndex: 'title', render: (title, item) => <Link className="font-bold text-emerald-700" to={employerAppPath(`/jobs/${item.public_id}`)}>{title || 'Tin nháp chưa đặt tên'}</Link> },
        ...
        { title: 'Hồ sơ', dataIndex: 'application_count', align: 'right', render: (count, item) => <Link to={employerAppPath(`/applications?job=${item.public_id}`)}>{count}</Link> },
