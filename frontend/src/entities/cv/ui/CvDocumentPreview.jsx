...
function SectionContent({ section, personal, assets, locale }) {
  if (section.section_key === 'nameplate') return <div><div className="text-2xl font-extrabold text-slate-900">{personal.full_name || getCvUiText('full_name', locale)}</div><div className="font-semibold text-[var(--cv-theme)]">{personal.headline || getCvUiText('headline', locale)}</div></div>
  if (section.section_key === 'contact') return <p className="text-slate-600">{[personal.email, personal.phone, personal.address].filter(Boolean).join(' · ') || getCvUiText('contact_info', locale)}</p>
  if (section.section_key === 'avatar') {
    const avatar = assets?.[personal.avatar_asset_id]
    return avatar?.url ? <img src={avatar.url} alt={getCvUiText('upload_avatar', locale)} className="h-24 w-24 rounded-full object-cover" /> : <div className="flex h-24 w-24 items-center justify-center rounded-full bg-slate-200 text-xs text-slate-500">{getCvUiText('upload_avatar', locale)}</div>
  }
  ...
  return <div className="space-y-2">{section.items.map((item) => <div key={item.item_id} data-cv-item-id={itemAttribute(item)}><p className="font-semibold text-slate-800">{item.name || item.degree || item.title || item.value || getCvUiText('content_item', locale)}</p><p className="text-slate-600">{item.institution || item.issuer || item.organization || item.role || ''}</p><div className="text-slate-700">{item.description ? <RichTextContent value={item.description} /> : null}</div></div>)}</div>
}

function PreviewSection({ section, themeColor, personal, assets, locale }) {
  if (!section.enabled) return null
  const definition = getSectionDefinition(section.section_key)
  return <section data-section-id={section.instance_id} className="cv-document-preview__section mb-5 break-inside-avoid">{!definition?.personalInfoBacked && <h3 className="mb-2 border-b pb-1 text-sm font-bold uppercase tracking-wide" style={{ color: themeColor, borderColor: `${themeColor}55` }}>{section.title || getSectionDisplayName(section.section_key, locale)}</h3>}<SectionContent section={section} personal={personal} assets={assets} locale={locale} /></section>
}
...
function RegionRows({ regions, contract, document, assets, renderRegion, pageNumber }) {
  ...
  return <div className="space-y-5">{contract.rows.map((row) => {
    ...
    return <div key={row.join(':')} className="grid gap-5" style={{ gridTemplateColumns: rowRegions.map((region) => `${region.widthPercent || 100}fr`).join(' ') }}>{rowRegions.map((region) => renderRegion
      ...
      : <div key={region.id}>{region.sections.map((section) => <PreviewSection key={section.instance_id} section={section} themeColor={document.style_json.theme_color} personal={personal} assets={assets} locale={document.content_json.locale} />)}</div>)}</div>
  })}</div>
}
  ...
  return (
    <div className="cv-document-preview__page-wrap relative mb-5">
      ...
      <article
        ...
        style={{ fontFamily: getCvFontStack(style_json.font_family), fontSize: `${style_json.font_scale}em`, lineHeight: style_json.line_height, backgroundImage: background?.url ? `url(${background.url})` : undefined, backgroundSize: 'cover', '--cv-theme': style_json.theme_color }}
      >
        {renderHeader ? renderHeader(page.number) : contract.key !== 'header_two_column_v1' && <header className="mb-6 border-l-4 pl-4" style={{ borderColor: style_json.theme_color }}>
          <h2 className="text-2xl font-extrabold text-slate-900">{personal.full_name || getCvUiText('full_name', content_json.locale)}</h2>
          <p className="mt-1 font-semibold" style={{ color: style_json.theme_color }}>{personal.headline || getCvUiText('headline', content_json.locale)}</p>
