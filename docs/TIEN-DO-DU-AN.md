...
Cập nhật lần cuối: 2026-07-17 (CVB-8 — tối ưu UX canvas theo khảo sát trực tiếp TopCV builder: toolbar section thu gọn nằm trong hàng tiêu đề, toolbar item chuyển xuống cạnh dưới-phải nên không còn che tiêu đề/nội dung đang gõ; mỗi lúc chỉ một tầng chrome hiển thị và toàn bộ chrome ẩn khi focus contenteditable (CSS `:has`); nút Xóa có nhãn + màu ngữ nghĩa, bỏ nút xóa nguy hiểm khỏi toolbar định dạng InlineText, toolbar InlineText kẹp mép phải viewport; thêm "Ảnh đại diện" giờ mặc định vào đầu cột phụ (fallback `header→sidebar` + `insertAtStart` trong registry) thay vì rơi cuối cột chính; thêm mục tự cuộn tới và focus ô đầu tiên; mục Mục tiêu nghề nghiệp bỏ ô "Tên" thừa (preview/PDF không in) và có placeholder gợi ý; placeholder ngày đổi sang dạng ví dụ `Từ (vd: 2020-01)`; nhãn vùng MAIN/SIDEBAR đổi thành Cột chính/Cột phụ ở panel Thêm mục/Bố cục; sửa bug antd `.ant-btn` đè Tailwind `absolute` khiến nút "+ Thêm nội dung" nằm trong flow làm section editor cao hơn bản in; trạng thái chọn section/item có viền theo theme; lint + architecture + 52 Vitest cv/edit-cv-draft + build pass. HOME-CV giữ nguyên tiến độ)
