...
Cập nhật lần cuối: 2026-07-19 (EMP-P1→P9 — hoàn tất cổng marketing, auth, `/employer-verify`, nhóm route account settings, dashboard nhà tuyển dụng, email chào mừng theo nguồn đăng ký, polish UX GPKD và trang DLCN. Luồng email/password chỉ chào mừng sau khi verify, Google mới chào mừng ngay và outbox chống gửi trùng ở DB. Employer đủ onboarding nhưng còn thiếu một trong năm điều kiện bảo mật/pháp lý sẽ vào `/employer-verify` và không bị `returnUrl=/dashboard` bỏ qua; khi `employer_verification_completed=true` thì đăng nhập vào thẳng dashboard hoặc deep-link an toàn. Bước đăng tin đầu tiên chưa triển khai không khóa trạng thái xác thực này. Luồng chuẩn `/account/verify → /consulting-need → /employer-verify → /dashboard`, consulting chỉ tạo một lần. Workspace `100dvh` đã được thiết kế lại theo mẫu quản trị với compliance bar, topbar, sidebar trắng, header route và content cuộn; desktop sidebar thu 216px→64px thành icon rail lớn/căn giữa, hover rail mở tạm và rời chuột tự thu, mobile vẫn ẩn khi đóng; dashboard có hành trình xác thực ngang nhưng không tạo action giả cho workflow chưa làm. Company chỉ hoàn tất sau khi chọn/tạo rõ ràng, hai bước DLCN được theo dõi độc lập. Trang GPKD hiển thị hai luồng tài liệu theo thứ tự radio dọc ổn định, ảnh mẫu local và link mẫu mới; chọn file chỉ là local UI, Lưu còn khóa trước khi persistence được bật. Trang DLCN dùng mẫu DOCX local, cho thay thế đúng một tệp và cho Lưu/Xác nhận ngay cả khi chưa có company; dữ liệu tài liệu mới gắn recruiter nhưng tương thích dữ liệu company cũ. Toàn bộ thông báo hệ thống đã chuyển sang Sonner, render độc lập khỏi workspace và phân biệt màu/biểu tượng theo trạng thái. Đã qua 39 backend regression, 261 frontend unit với coverage 87,10%, lint/architecture/build và 18 smoke desktop/mobile xanh cho cổng employer. Full backend 229 test còn 5 lỗi độc lập trong `apps.applications.tests_migrations` do model hiện tại truy vấn cột `contact_name` khi schema test đang ở migration legacy; lỗi tái hiện khi chạy riêng suite này và không thuộc phạm vi EMP-P9. Các cập nhật 1.24b→1.24d, CVB-8 và HOME-CV giữ nguyên tiến độ.)
