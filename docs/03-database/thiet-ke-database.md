...
| `candidate_email_notification_settings` | `backend/apps/candidates` | 12 opt-in email candidate, one-to-one profile, mặc định tắt; email bảo mật không thuộc bảng |
...
- `candidate_email_notification_settings` là policy preference, không phải hàng
  đợi gửi mail. Producer email sản phẩm bổ sung sau phải kiểm tra opt-in ngay
  trước delivery, còn email xác thực/reset mật khẩu/2FA tiếp tục đi qua luồng
  bảo mật riêng và không được tắt ở trang này.
