...
- Không có luồng onboarding theo các mốc độc lập (xác thực SĐT; thông tin công ty; giấy ĐKDN; thỏa thuận DLCN; đăng tin đầu tiên).
...
5. **Đã xong (2026-07-20):** P2–P5 ở frontend/product; tiếp tục chuẩn hóa app nhỏ theo R4 khi xuất hiện nhu cầu thực tế.
...
## 8. Catalogue và form công ty hoàn thiện (2026-07-20)
...
- Logo/gallery nhận JPG/PNG/WebP hợp lệ tối đa 5 MB. Gallery tối đa 10 ảnh, `sort_order` tăng dần; 3:2 và 1200×800 chỉ là khuyến nghị. Storage key mới được dọn nếu thao tác DB thất bại.
- Owner quản lý logo/gallery và gửi thay đổi text qua `CompanyUpdateRequest`; member approved/pending chỉ xem.
- Đổi MST hoặc tên đăng ký bắt buộc lý do, `proof_type` và tài liệu gắn `update_request`. Service admin không approve request nhạy cảm nếu thiếu bộ hồ sơ tương ứng.
- Frontend tổ chức trong `features/manage-employer-company`: orchestration, search panel, create/edit form, linked-company panel và model diff. Rich editor domain-agnostic nằm ở `shared/ui/RichTextEditor`.
