...
- Admin chỉ hiển thị boolean `env_configured` qua cùng resolver; không expose
  provider secret vào API hoặc UI.
...
| CVB-0.2 | CV API V1→V2 cutover | 🟡 Đang đo | V2 có hard-delete/import/metadata; V1 có deprecation headers + telemetry, chưa đủ điều kiện 410/xóa |
...
| CVB-3 | Candidate “My CV” | ✅ Hoàn tất | Hard-delete/default/rename/import/duplicate; snapshot ứng tuyển retained detached; smoke desktop/mobile phủ workflow chính; CTA mở owner view với export PDF immutable |
...
- [x] Hard-delete/default/rename CV bằng service transaction và permission owner;
  draft/version/share/export/file library bị xóa, còn application snapshot được
  detached để bảo toàn hồ sơ tuyển dụng; DB chặn nhiều default active.
