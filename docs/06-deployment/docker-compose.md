...
- **Queue Celery**: settings route task sang 5 queue (`default`, `auth-email`,
  `cv-export`, `speech-artifacts`, `upload-scan`). Worker trong compose khai đủ
  cả năm; bỏ queue tương ứng thì email, export CV, upload MP3 TTS hoặc malware
  scan/cleanup có thể không chạy.
