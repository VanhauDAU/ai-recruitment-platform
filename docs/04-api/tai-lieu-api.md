...
| GET | `/api/companies/?q=&cursor=` | Danh bạ công khai. Không `q`: tối đa 18 company featured đã qua eligibility, finite và không có `count`. Có `q`: tìm không dấu trên tên/tên thương mại trong mọi `Company`, không áp dụng verification/completeness gate; trang đầu trả exact `count`, trang cursor sau bỏ `count`. DTO public-safe không trả MST/email/phone/raw address/recruiter và luôn redact địa chỉ hộ kinh doanh. |
...
| GET | `/api/jobs/recommendations/for-me/?page=&page_size=` | Candidate-only: feed chỉ dùng nhu cầu công việc đã lưu (vị trí taxonomy/tự nhập, kỹ năng, lương, kinh nghiệm, tỉnh, relocate), có setup/consent và phân trang. `sources.cv=false`, `source_cv=null`; loại job đã ứng tuyển/đã ẩn trước cap. |
...
| GET | `/api/jobs/recommendations/inline/?page=&ranking_seed=&excluded=` | Candidate-only: `ranking_seed` bắt buộc; lane độc lập có xác suất deterministic 40% theo candidate/seed/page; trả `not_shown` hoặc 1–2 job cùng `after_result_index` zero-based. `excluded` là CSV tối đa 50 public ID; luôn loại tin đã ứng tuyển/đã ẩn. |
| POST | `/api/jobs/recommendations/hidden/` | Candidate-only, idempotent: body `{job_public_id, source}` với source `matching`, `homepage` hoặc `inline`; ẩn đúng job khỏi các bề mặt cá nhân hóa. |
| DELETE | `/api/jobs/recommendations/hidden/{job_public_id}/` | Candidate-only, owner-scoped và idempotent: hoàn tác ẩn; không tác động bản ghi ẩn của ứng viên khác. |
